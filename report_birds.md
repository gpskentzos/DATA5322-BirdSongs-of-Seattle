...
**Dataset.** 1,981 recordings across 12 Pacific Northwest species, sourced from Xeno-Canto (Vellinga and Planque, 2015) and filtered by the instructor for quality and duration. Data are stored in HDF5 format as (128, 517, N) arrays per species. Table 1 lists the species and recording counts.
...
**Splits.** Stratified 70/15/15 split: 1,383 training, 299 validation, 299 test samples.

**Binary task.** Bewick's Wren (144) vs. Dark-eyed Junco (125). Two architectures: Architecture A (3 conv blocks + GAP + Dense(64), no regularization, 100,993 params) and Architecture B (same backbone + Dropout(0.25) between blocks + Dropout(0.40) in head, 109,313 params).

**Multi-class task.** All 12 species with class weighting. Three architectures share the same 3-conv backbone: (1) Baseline CNN, (2) CNN + Dropout, (3) CNN + BatchNorm.

**Transfer learning.** EfficientNet-B0 (Tan and Le, 2019) pretrained on ImageNet. Single-channel spectrograms are replicated to 3 channels and rescaled to [0, 255] for compatibility with ImageNet weights. Phase 1: backbone frozen, head-only training at lr = 1e-3. Phase 2: top 40 of 238 backbone layers unfrozen, full network at lr = 1e-4.

**Test-clip analysis.** Three unlabeled MP3 clips (23.3 s, 5.3 s, 15.9 s) processed with a 3-second sliding window (1-second stride). Each window is classified by the best model. A second species is flagged if its mean window probability exceeds 0.20.
...
![Preprocessing validation](fig_test_preprocessing_validation.png)
...
The primary limitation is dataset size relative to input dimensionality. With 1,383 training samples across 12 classes (average 115 per class, minimum 25 for Northern Flicker) and a 66,176-dimensional spectrogram input, from-scratch CNNs cannot learn generalizable features. A bird call typically occupies 0.5--1 second of a 3-second window, leaving the majority of each spectrogram as background noise at the −80 dB floor -- further diluting the gradient signal per sample. EfficientNet succeeds where the custom CNNs fail precisely because its pretrained weights provide strong feature detectors that require only a few examples to redirect toward spectrogram patterns, rather than learning from noise.
